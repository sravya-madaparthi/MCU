...
void EXTI15_10_IRQn_Handler(void) {
    if (EXTI->PR & EXTI_PR_PR13) {  // Check if EXTI13 is triggered
        ...
        if ((GPIOC->IDR & GPIO_IDR_ID13) == 0) {  // Button still pressed
            ...
            uint8_t led1_toggles = press_count;
            uint8_t led2_toggles = press_count * 3;

            LED_Toggle(led1_toggles, led2_toggles);
        }
...
void LED_Toggle(uint8_t led1_count, uint8_t led2_count) {
    uint8_t max_toggles = (led1_count > led2_count) ? led1_count : led2_count;

    for (uint8_t i = 0; i < max_toggles; i++) {
        if (i < led1_count) {
            GPIOA->ODR ^= (1 << 5);  // Toggle LED1
            delay_ms(300);
            GPIOA->ODR ^= (1 << 5);
            delay_ms(300);
        }
        if (i < led2_count) {
            GPIOA->ODR ^= (1 << 7);  // Toggle LED2
            delay_ms(200);
            GPIOA->ODR ^= (1 << 7);
            delay_ms(200);
        }
